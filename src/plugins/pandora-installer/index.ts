...
export class PandoraInstaller extends TwLitElement {
  ...
  private handleOnCreateNoteClick() {
    const note = prompt("Enter the link you want to create") ?? "";
    ...
    if (note) {
      const data = {
        ...
        type: "pandora-note",
        attributes: {
          id: `pandora-note-${note.toString()}`,
          label: note,
          component: "pandora-note",
          icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 256 256"><path d="M208,32H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H156.69A15.92,15.92,0,0,0,168,219.31L219.31,168A15.92,15.92,0,0,0,224,156.69V48A16,16,0,0,0,208,32ZM96,88h64a8,8,0,0,1,0,16H96a8,8,0,0,1,0-16Zm32,80H96a8,8,0,0,1,0-16h32a8,8,0,0,1,0,16ZM96,136a8,8,0,0,1,0-16h64a8,8,0,0,1,0,16Zm64,68.69V160h44.7Z"></path></svg>`,
          note: note,
        },
      };

      document.dispatchEvent(
        new CustomEvent("pandora::register-plugin", {
          detail: data,
          bubbles: true,
        })
      );
...
customElements.whenDefined("pandora-box").then(() => {
  (customElements.get("pandora-box") as any).registerPlugin({
    ...
    hooks: {
      onReady: () => console.log("Installer is initialized!"),
      onMount: (el: Element) => console.log("Installer is mounted!"),
      onUnmount: () => console.log("Installer is destroyed"),
    },
  });
});
